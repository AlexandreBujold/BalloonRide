...
public class PlayerController : MonoBehaviour
{
    ...
    [Range(0, 25)]
    public float yAxialMaxSpeed = 10f;
    public float yAxialAccel = 1f;
    public float yAxialDecel = 1f;
    ...
    void Update()
    {
        ...
        movementValue = new Vector3(movementValue.x, movementValue.y + CalculateGravityAndJumpMovement(jump));

        //Apply Values
        transform.Translate(movementValue, Space.Self);
    ...
    public Vector3 CalculatePlanarMovement(Vector2 controlInput, bool constantForwardAccel)
    {
        ...
        if (constForwardMovement == true)
        {
            ...
            velocity.y = yAxialMaxSpeed;
            if (rotationTargetY != null && rotTargetRelMovement == true)
            {
                displacement = (rotationTargetY.up * controlInput.y * velocity.z);
            }
            else
            {
                displacement = (transform.up * controlInput.y * velocity.z);
            }
        ...
        else
        {
            ...
            {
                calculationDirection.y = movementInput.y >= 0 ? 1 : -1;
                velocity.y = AccelerateInAxis(calculationDirection.y, yAxialMaxSpeed, yAxialAccel, ref velocity.y);
            }
            else //No input, decelerate
            {
                velocity.y = DecelerateInAxis(calculationDirection.y, yAxialMaxSpeed, yAxialDecel, ref velocity.y);
            }
            ...
            if (cameraRelMovement == true)
            {
                if (gameCamera != null)
                {
                    ...
                    {
                        horizontalForward = rotationTargetY.up;
